...
from src.lab_3 import plot_utils
...
class RBMBase:
    ...
    def rbm_block(self, inputs, layer_from, layer_to, vis_unit_type='gauss'):
        ...
        bias_layer_from_upd = bias_layer_from.assign_add(
            tf.multiply(learning_rate, tf.reduce_mean(
                tf.subtract(visib_inputs_initial, vprob_last), 0)
            )
        )
        ...
        if self.bin_type:
            bias_layer_to_sub = tf.subtract(hstate_first, hstate_last)
        else:
            bias_layer_to_sub = tf.subtract(hprob_first, hprob_last)
    ...
    @staticmethod
    def _binary_from_prob(probs):
        """Return binary  0 or 1"""
        return tf.round(probs);

    def _sample_hidden_from_visible(self, visible_units, hid_layer_no,recon):
       ...
       # else:
       #     hid_states = self._binary_from_prob(hid_probs)
    ...
    def _gibbs_sampling_step(self, visible, hid_layer_no=1):
        ...
        hprobs, hstates = self._sample_hidden_from_visible(
            visible, hid_layer_no,recon = False)
        ...
        hprobs1, hstates1 = self._sample_hidden_from_visible(
            vprobs, hid_layer_no,recon = True)
    ...
    def _epoch_train_step(self, output_summary,calculate_free_energy =False):
        params = self.params
        ...
        valid_batches = self.data_provider.get_validation_set_iter(
            params['batch_size'], params['shuffle'])
        train_sub_set = self.data_provider.train.train[0:params['free_energy_batch_size'],:];
        validation_sub_set = self.data_provider.test.train[0:params['free_energy_batch_size'],:];
        ...
        for batch_no, train_batch in enumerate(batches):
            ...
            if batch_no % 11 == 0 and self.params['validate']:
                ...
                valid_inputs= validation_sub_set;
                ...
                if(calculate_free_energy):
                    free_energy_val = self.sess.run(
                        self.free_energy, feed_dict=feed_dict)
                    feed_dict_train_sub = {self.inputs: train_sub_set}
                    free_energy_train = self.sess.run(
                        self.free_energy, feed_dict=feed_dict_train_sub)
            #  print(valid_loss)
                #valid_loss = float(valid_loss)
                #summary=tf.Summary()
                #summary.value.add(tag='myVar', simple_value = valid_loss)
                #summary_str = tf.Summary(value=tf.Summary.Value(valid_loss))
                output_summary.append(( self.params['global_step'],float(valid_loss),float(free_energy_train),float(free_energy_val)));
                #tf.summary.FileWriter.add_summary( valid_loss, self.params['global_step'])
    ...
    def _get_embeddings(self, sess, run_no, train_set=False,
        ...
        for i in range(30, np.size(test_data.train,0)):
            target  =int(test_data.target[i]);
            if(not has_number[target]):
                examples.append(test_data.train[i,:]);
    ...
    def _get_embeddings_2(self, sess, run_no, train_set=False,
                        plot_images=False):
        batch_size = self.params['batch_size']
        if not train_set:
